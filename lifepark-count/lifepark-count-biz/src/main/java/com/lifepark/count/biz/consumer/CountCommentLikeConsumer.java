...
import com.lifepark.count.biz.enums.LikeUnlikeCommentTypeEnum;
import com.lifepark.count.biz.model.dto.AggregationCountLikeUnlikeCommentMqDTO;
import com.lifepark.count.biz.model.dto.CountLikeUnlikeCommentMqDTO;
...
@Component
@RocketMQMessageListener(consumerGroup = "lifepark_group_count_"
        + MQConstants.TOPIC_COMMENT_LIKE_OR_UNLIKE, // Group 组
...
public class CountCommentLikeConsumer implements RocketMQListener<String> {
    ...
    private void consumeMessage(List<String> bodys) {
        ...
        List<CountLikeUnlikeCommentMqDTO> countLikeUnlikeCommentMqDTOS = bodys
                .stream()
                .map(body -> JsonUtils.parseObject(body, CountLikeUnlikeCommentMqDTO.class))
                .toList();

        // 按评论 ID 进行分组
        Map<Long, List<CountLikeUnlikeCommentMqDTO>> groupMap = countLikeUnlikeCommentMqDTOS
                .stream()
