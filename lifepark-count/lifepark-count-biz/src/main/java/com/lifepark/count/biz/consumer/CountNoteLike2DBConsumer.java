...
//
//@Component
//@RocketMQMessageListener(consumerGroup = "lifepark_group_" + MQConstants.TOPIC_COUNT_NOTE_LIKE_2_DB, // Group 组
...
//计数: 笔记点赞数落库到 笔记统计表中获赞总数与用户统计表的获赞总数

@Component
@RocketMQMessageListener(consumerGroup = "lifepark_group_"
        + MQConstants.TOPIC_COUNT_NOTE_LIKE_2_DB, // Group 组
        topic = MQConstants.TOPIC_COUNT_NOTE_LIKE_2_DB // 主题 Topic
)
@Slf4j
public class CountNoteLike2DBConsumer implements RocketMQListener<String> {
    ...
    public void onMessage(String body) {
        ...
        if (CollUtil.isNotEmpty(countList)) {
            // 判断数据库中 t_user_count 和 t_note_count 表
            // 若笔记计数记录不存在，则插入；若记录已存在，则直接更新
