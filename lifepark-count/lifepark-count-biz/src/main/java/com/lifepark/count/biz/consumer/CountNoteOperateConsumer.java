...
import com.lifepark.count.biz.model.dto.NoteOperateMqDTO;
...
@Component
@RocketMQMessageListener(consumerGroup = "lifepark_group_" + MQConstants.TOPIC_NOTE_OPERATE ,
...
public class CountNoteOperateConsumer implements RocketMQListener<Message> {
    ...
    public void onMessage(Message message) {
        //解析消息体  Messgae----String-----NoteOperateMqDTO
        String string = new String(message.getBody());
        NoteOperateMqDTO noteOperateMqDTO = JsonUtils.parseObject(string, NoteOperateMqDTO.class);
        //根据标签判断类型 （发布笔记还是删除笔记）
        String tags = message.getTags();
        log.info("==> CountNotePublishConsumer 消费了消息 {}, tags: {}", string, tags);
        // 根据 MQ 标签，判断笔记操作类型
        if (Objects.equals(tags, MQConstants.TAG_NOTE_PUBLISH)) { // 笔记发布
            handleTagMessage(string, 1);
