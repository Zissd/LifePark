...
import com.lifepark.count.biz.enums.CollectUnCollectNoteTypeEnum;
import com.lifepark.count.biz.model.dto.AggregationCountCollectUnCollectNoteMqDTO;
import com.lifepark.count.biz.model.dto.CountCollectUnCollectNoteMqDTO;
...
@Component
@RocketMQMessageListener(consumerGroup = "lifepark_group_" + MQConstants.TOPIC_COUNT_NOTE_COLLECT, // Group 组
...
//                .build();
        ...
        countList.forEach(item -> {
            ...
            // 若存在才会更新，因为缓存设有过期时间，笔记维度 Hash 初始化工作放在查询计数来做
            if (isCountNoteExisted) {
                // 更新笔记维度 Hash 的收藏总数字段
                redisTemplate.opsForHash()
                        .increment(countNoteRedisKey, RedisKeyConstants.FIELD_COLLECT_TOTAL, count);
            }
            ...
            if (isCountUserExisted) {
                // 更新用户维度 Hash 被收藏总数字段
                redisTemplate.opsForHash()
                        .increment(countUserRedisKey, RedisKeyConstants.FIELD_COLLECT_TOTAL, count);
            }
        });
        // 发送 MQ, 笔记收藏数据落库
