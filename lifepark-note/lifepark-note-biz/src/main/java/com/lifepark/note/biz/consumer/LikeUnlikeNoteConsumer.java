...
//
//@Component
//@RocketMQMessageListener(consumerGroup = "lifepark_group_" + MQConstants.TOPIC_LIKE_OR_UNLIKE, // Group 组
...
//                .build();
//
//        // 异步发送 MQ 消息 主题是 计数 - 笔记点赞数 与用户获赞数
...
//               //实体中type：0是取消点赞，1是点赞
//               //数据库中：0是取消点赞，1是点赞
//               //到数据库中查找status状态是 1的点赞记录，把状态改为 0 ，逻辑上取消点赞
...
public class LikeUnlikeNoteConsumer {
    ...
    public DefaultMQPushConsumer mqPushConsumer() throws MQClientException {
        // Group 组
        String group = "lifepark_group1_" + MQConstants.TOPIC_LIKE_OR_UNLIKE;
        ...
        consumer.registerMessageListener((MessageListenerOrderly) (msgs, context) -> {
            ...
            try {
                ...
                // 1. 内存级操作合并
                // 按用户 ID 进行分组
                Map<Long, List<LikeUnlikeNoteMqDTO>> groupMap = likeUnlikeNoteMqDTOS
                        .stream()
                        .collect(Collectors.groupingBy(LikeUnlikeNoteMqDTO::getUserId));

                // 对每个用户的操作按 noteId 二次分组，并过滤合并
                List<LikeUnlikeNoteMqDTO> finalOperations = groupMap
                        .values()
                        .stream()
                ...
                if (CollUtil.isNotEmpty(finalOperations)) {
                    // DTO 转 DO
                    List<NoteLikeDO> noteLikeDOS = finalOperations
                            .stream()
