...
import com.lifepark.comment.biz.enums.LikeUnlikeCommentTypeEnum;
...
public class LikeUnlikeComment2DBConsumer {
    ...
    public DefaultMQPushConsumer mqPushConsumer() throws MQClientException {
        // Group 组
        String group = "lifepark_group_" + MQConstants.TOPIC_COMMENT_LIKE_OR_UNLIKE;
        ...
        consumer.registerMessageListener((MessageListenerOrderly) (msgs, context) -> {
            ...
            try {
                ...
                // 按评论 ID 分组
                Map<Long, List<LikeUnlikeCommentMqDTO>> commentIdAndListMap = likeUnlikeCommentMqDTOS
                                .stream()
...
// 若某个用户对某评论，多次操作，如点赞 -> 取消 -> 点赞，需进行操作合并，只提取最后一次操作，降低操作数据库的频率
                    Map<Long, LikeUnlikeCommentMqDTO> userLastOp = ops
                            .stream()
    ...
    private void executeBatchSQL(Collection<LikeUnlikeCommentMqDTO> values) {
        // 过滤出点赞操作
        List<LikeUnlikeCommentMqDTO> likes = values
                .stream()
                .filter(op -> Objects.equals(op.getType(), LikeUnlikeCommentTypeEnum.LIKE.getCode()))
                .toList();

        // 过滤出取消点赞操作
        List<LikeUnlikeCommentMqDTO> unlikes = values
                .stream()
