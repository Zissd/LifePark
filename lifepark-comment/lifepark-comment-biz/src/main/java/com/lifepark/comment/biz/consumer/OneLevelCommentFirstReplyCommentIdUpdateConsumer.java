...
@Component
@RocketMQMessageListener(consumerGroup = "lifepark_group_first_reply_comment_id"
...
public class OneLevelCommentFirstReplyCommentIdUpdateConsumer implements RocketMQListener<String> {
    ...
    private void consumeMessage(List<String> bodys) {
        ...
        // 过滤出二级评论的 parent_id（即一级评论 ID），并去重，需要这些更新一级评论的 first_reply_comment_id
        List<Long> parentIds = publishCommentMqDTOS
                .stream()
        ...
        // 构建 Redis Key
        List<String> keys = parentIds
                .stream()
                .map(RedisKeyConstants::buildHaveFirstReplyCommentKey)
                .toList();

        // 批量查询 Redis 中 keys 列表对应的值列表
        ...
        //missingCommentIds存储的是待更新的一级评论ID，需要去数据库查询
        if (CollUtil.isNotEmpty(missingCommentIds)) {
            ...
            threadPoolTaskExecutor.submit(() -> {
                List<Long> needSyncCommentIds = commentDOS
                        .stream()
            ...
            // 过滤出first_reply_comment_id值为 0 的，都需要更新其 first_reply_comment_id
            List<CommentDO> needUpdateCommentDOS = commentDOS
                    .stream()
                    .filter(commentDO -> commentDO.getFirstReplyCommentId() == 0)
                    .toList();
            needUpdateCommentDOS.forEach(needUpdateCommentDO -> {
                ...
                CommentDO earliestCommentDO = commentDOMapper
                        .selectEarliestByParentId(needUpdateCommentId);
