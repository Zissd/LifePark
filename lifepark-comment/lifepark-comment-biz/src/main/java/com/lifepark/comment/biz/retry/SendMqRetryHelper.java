package com.lifepark.comment.biz.retry;
...
public class SendMqRetryHelper {
        ...
        threadPoolTaskExecutor.submit(() -> {
            try {
                ...
                retryTemplate.execute((RetryCallback<Void, RuntimeException>) context -> {
                    log.info("==> 开始重试 MQ 发送, 当前重试次数: {}, 时间: {}",
                            context.getRetryCount() + 1, LocalDateTime.now());
                    // 同步发送 MQ
    ...
    // 兜底方案: 将发送失败的 MQ 写入数据库，
    // 之后，通过定时任务扫表，将发送失败的 MQ 再次发送，最终发送成功后，将该记录物理删除
