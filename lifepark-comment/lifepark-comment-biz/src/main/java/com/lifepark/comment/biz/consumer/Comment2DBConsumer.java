package com.lifepark.comment.biz.consumer;
...
import com.lifepark.comment.biz.enums.CommentLevelEnum;
import com.lifepark.comment.biz.model.bo.CommentBO;
import com.lifepark.comment.biz.model.dto.CountPublishCommentMqDTO;
...
public class Comment2DBConsumer {
    ...
    // 执行编程式事务逻辑 transactionTemplate.execute(status->{......});
    //比声明式事务 @Transactional 更灵活，适合需要细粒度控制事务的场景。
    @Resource
    ...
    @Bean  //自定义创建 DefaultMQPushConsumer
    public DefaultMQPushConsumer mqPushConsumer() throws MQClientException {
        // Group 组
        String group = "lifepark_group_" + MQConstants.TOPIC_PUBLISH_COMMENT;
        ...
        consumer.registerMessageListener((MessageListenerConcurrently) (msgs, context) -> {
            ...
            try {
                ...
                msgs.forEach(msg -> {
                    ...
                    publishCommentMqDTOS
                            .add(JsonUtils.parseObject(msgJson, PublishCommentMqDTO.class));
                });

                // 提取所有不为空的回复评论 ID
                List<Long> replyCommentIds = publishCommentMqDTOS
                        .stream()
                ...
                for (PublishCommentMqDTO publishCommentMqDTO : publishCommentMqDTOS) {
                    ...
                    //先创建出默认的 BO 对象 相关字段设置默认值，再逐步修改部分信息
                    CommentBO commentBO = CommentBO
                            .builder()
                    ...
                    if (StringUtils.isNotBlank(content)) {
                        // 生成要储存在cassnandra中的评论内容的 UUID 标识
                    ...
                    if (Objects.nonNull(replyCommentId)) {
                        ...
                        if (Objects.nonNull(replyCommentDO)) {
                            ...
                            //如果回复的评论也属于二级评论，则将父评论 ID 设置为该评论的父评论 ID
                ...
                // 编程式事务transactionTemplate，保证整体操作的原子性
// 注意：跨异构数据库（MySQL + Cassandra）的操作无法保证整体原子性——
// Spring 的 TransactionTemplate 只能管理单一数据源，覆盖不到 Cassandra；
// 且 Cassandra 不支持事务回滚，批量操作失败后无法撤销已写入的数据。
                Integer insertedRows = transactionTemplate.execute(status -> {
                    try {
                        // 1.先批量存入mysql数据库评论元数据
                        int count = commentDOMapper.batchInsert(commentBOS);

                        // 过滤出评论内容不为空的 BO
                        List<CommentBO> commentContentNotEmptyBOS = commentBOS
                                .stream()
                                .filter(commentBO -> Boolean.FALSE.equals(commentBO.getIsContentEmpty()))
                                .toList();
                        if (CollUtil.isNotEmpty(commentContentNotEmptyBOS)) {
                            // 2.再批量存入 cassandra 评论内容
                ...
                if (Objects.nonNull(insertedRows) && insertedRows > 0) {
                    // 构建发送给计数服务的 DTO 集合
                    List<CountPublishCommentMqDTO> countPublishCommentMqDTOS = commentBOS
                            .stream()
                    ...
                    // 异步发送 MQ 计数消息
//这个主题的消息会被两个 MQ 接收，一个统计笔记的评论总数，另一个统计一级评论的子评论总数（即某一级评论的二级评论总数）
                    rocketMQTemplate.asyncSend
                            (MQConstants.TOPIC_COUNT_NOTE_COMMENT, message, new SendCallback() {
                        @Override
