package com.lifepark.comment.biz.service.impl;
...
import com.lifepark.comment.biz.constant.MQConstants;
import com.lifepark.comment.biz.constant.RedisKeyConstants;
import com.lifepark.comment.biz.domain.dataobject.CommentDO;
import com.lifepark.comment.biz.domain.dataobject.CommentLikeDO;
import com.lifepark.comment.biz.domain.mapper.CommentDOMapper;
import com.lifepark.comment.biz.domain.mapper.CommentLikeDOMapper;
import com.lifepark.comment.biz.domain.mapper.NoteCountDOMapper;
import com.lifepark.comment.biz.enums.*;
import com.lifepark.comment.biz.model.dto.LikeUnlikeCommentMqDTO;
import com.lifepark.comment.biz.model.dto.PublishCommentMqDTO;
import com.lifepark.comment.biz.model.vo.*;
import com.lifepark.comment.biz.retry.SendMqRetryHelper;
import com.lifepark.comment.biz.rpc.DistributedIdGeneratorRpcService;
import com.lifepark.comment.biz.rpc.KeyValueRpcService;
import com.lifepark.comment.biz.rpc.UserRpcService;
import com.lifepark.comment.biz.service.CommentService;
import com.lifepark.kv.dto.req.FindCommentContentReqDTO;
...
public class CommentServiceImpl implements CommentService {
    ...
    public Response<?> publishComment(PublishCommentReqVO publishCommentReqVO) {
        ...
        Preconditions.checkArgument(
                StringUtils.isNotBlank(content) || StringUtils.isNotBlank(imageUrl),
                "评论正文和图片不能同时为空");
        // 发布者 ID
...
//        });
        // 发送 MQ (包含重试机制)
        sendMqRetryHelper.asyncSend(MQConstants.TOPIC_PUBLISH_COMMENT,
                JsonUtils.toJsonString(publishCommentMqDTO));//异步发送MQ
    ...
    public PageResponse<FindCommentItemRspVO> findCommentPageList(FindCommentPageListReqVO findCommentPageListReqVO)
    {
        ...
        // 评论分页缓存使用 ZSET + STRING 实现    ZSET存放笔记下的评论id集合，String存放评论详情
        // 构建评论 ZSET Key
        ...
        // 若不存在
        if (!hasKey) {
            // 异步将笔记的热点评论同步到 redis 中（最多同步 500 条）
            threadPoolTaskExecutor.execute(() ->
                    syncHeatComments2Redis(commentZSetKey, noteId));
        }

        // 若 ZSET 缓存存在, 并且查询的是前 50 页的评论 超过50页的使用数据库查询 缓存资源宝贵
        if (hasKey && offset < 500) {
            // 使用 ZRevRange 分页查询某篇笔记下，按热度降序排序的一级评论 ID
            ...
            if (CollUtil.isNotEmpty(commentIds)) {
                ...
                // 构建查询本地缓存的 Key 集合 。String 转 Long
                List<Long> localCacheKeys = commentIdList
                        .stream()
                        .map(commentId -> Long.valueOf(commentId.toString()))
                        .toList();
                // 批量查询评论详情本地缓存 并对缓存中没有的id进行记录
                Map<Long, String> commentIdAndDetailJsonMap = LOCAL_CACHE
                        .getAll(localCacheKeys, missingKeys -> {
                ...
                // 若 localCacheExpiredCommentIds 不等于 commentIdList 的大小，说明本地缓存查到了数据
                ...
                //若localCacheExpiredCommentIds大小等于 0，说明评论详情数据在本地缓存都查到，直接响应返参
                if (CollUtil.size(localCacheExpiredCommentIds) == 0) {
                    // 评论的计数数据需要从 Redis Hash中查 设置到commentRspVOS中
                ...
                // 构建 MGET 批量查询本地缓存缺失的评论详情的 Key 集合
                List<String> commentIdKeys = localCacheExpiredCommentIds  //commentIdList
                        .stream()
                ...
                // 对于缓存中存在的评论详情, 需要去评价的 Hash 查询其计数数据（获赞数，二级评价总数）
                if (CollUtil.isNotEmpty(commentRspVOS)) {
                    setCommentCountData(commentRspVOS, expiredCommentIds);
                }

                // 对于redis中不存在的一级评论，需要批量从数据库中查询，同步redis并添加到commentRspVOS中
...
//        // 过滤出所有最早回复的二级评论 ID
//        List<Long> twoLevelCommentIds = oneLevelCommentDOS
//                .stream()
...
//        }
        ...
        // 异步将评论详情，同步到本地缓存
        syncCommentDetail2LocalCache(commentRspVOS);//异步将评论详情，同步到本地缓存
    ...
    (FindChildCommentPageListReqVO findChildCommentPageListReqVO) {
        ...
        if (hasKey && offset < 6*10) {
            ...
            if (CollUtil.isNotEmpty(childCommentIds)) {
                ...
                List<String> commentIdKeys = childCommentIds
                        .stream()
                        .map(RedisKeyConstants::buildCommentDetailKey)
                        .toList();
    ...
    public Response<?> likeComment(LikeCommentReqVO likeCommentReqVO) {
        ...
        script.setScriptSource(new ResourceScriptSource(
                new ClassPathResource("/lua/bloom_comment_like_check.lua")));
        // 返回值类型
        ...
        // 构建消息对象，并将 DTO 转成 Json 字符串设置到消息体中
        Message<String> message = MessageBuilder
                .withPayload(JsonUtils.toJsonString(likeUnlikeCommentMqDTO))
                .build();
        ...
        // 异步发送 MQ 消息，提升接口响应速度
        // 会有两个 MQ 接收到该主题消息 一个进行点赞的落库，另一个进行点赞的计数
    ...
    public Response<?> deleteComment(DeleteCommentReqVO deleteCommentReqVO) {
        ...
        // 4. 删除 Redis 缓存（ZSet id列表 和 String）
    ...
    /**
     * 获取全部评论数据，并将评论详情同步到 Redis 评论详情String中
    ...
    /**
     * 获取某评论下子评论列表，并将子评论的全部信息同步到 Redis String中
    ...
    (List<CommentDO> childCommentDOS, List<FindChildCommentItemRspVO> childCommentRspVOS) {
        ...
        for (CommentDO childCommentDO : childCommentDOS) {
            ...
            // 若当前评论的 replyCommentId 不等于 parentId，则前端需要展示回复的哪个用户，如  “回复 张三：”
