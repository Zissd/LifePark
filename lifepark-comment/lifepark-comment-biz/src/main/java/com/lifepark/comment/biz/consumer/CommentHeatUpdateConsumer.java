...
import com.lifepark.comment.biz.model.bo.CommentHeatBO;
import com.lifepark.comment.biz.util.HeatCalculator;
...
@RocketMQMessageListener(consumerGroup = "lifepark_group_"
        + MQConstants.TOPIC_COMMENT_HEAT_UPDATE, // Group 组
...
public class CommentHeatUpdateConsumer implements RocketMQListener<String> {
    ...
    private void consumeMessage(List<String> bodys) {
        ...
        // 根据评论id列表批量查询评论
        List<CommentDO> commentDOS = commentDOMapper
                .selectByCommentIds(commentIds.stream().toList());
    ...
    private void updateRedisHotComments(List<CommentHeatBO> commentHeatBOList) {
        ...
        Map<Long, List<CommentHeatBO>> noteIdAndBOListMap = commentHeatBOList
                .stream()
        ...
        noteIdAndBOListMap.forEach((noteId, commentHeatBOS) -> {
            ...
            script.setScriptSource(new ResourceScriptSource(
                    new ClassPathResource("/lua/update_hot_comments.lua")));
            // 返回值类型
