package com.lifepark.search.canal;
...
import com.lifepark.search.domain.mapper.SelectMapper;
import com.lifepark.search.enums.NoteStatusEnum;
import com.lifepark.search.enums.NoteVisibleEnum;
...
//            }
    ...
    {
        //columnMap是一行中所有列的数据  eventType是操作类型（insert，update（含删除，逻辑删除））
        // 获取笔记 ID
    ...
    {
        //这里设置成List是selectEsNoteIndexData返回值为List，用户表改变时会影响他的es笔记索引数据

        // 从数据库查询 Elasticsearch 索引数据 当前笔记id对应的新增的笔记信息
    ...
    private void handleUserEvent(Map<String, Object> columnMap, CanalEntry.EventType eventType)
            throws Exception {
        //用户表出现修改操作时，需要刷新es中用户索引和笔记索引，因为两个索引都有用户表的部分信息
            ...
            case UPDATE -> { // 记录更新事件
                ...
                if (Objects.equals(status, StatusEnum.ENABLE.getValue())
                    ...
                    syncNotesIndexAndUserIndex(userId);
                        ...
                        || Objects.equals(isDeleted, 1)) { // 或者被逻辑删除
                    // 删除es中对应用户的文档
