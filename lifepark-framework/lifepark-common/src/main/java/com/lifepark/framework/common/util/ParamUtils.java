package com.lifepark.framework.common.util;
...
public final class ParamUtils {
     ...
     * @return
     */
    public static boolean checkLifeparkId(String lifeparkId) {
        // 检查长度
        if (lifeparkId.length() < ID_MIN_LENGTH || lifeparkId.length() > ID_MAX_LENGTH) {
            return false;
        }
        // 检查格式
        Pattern pattern = Pattern.compile(ID_REGEX);
        return pattern.matcher(lifeparkId).matches();
    }
