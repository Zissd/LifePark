...
import com.lifepark.data.align.model.dto.FollowUnfollowMqDTO;
...
@RocketMQMessageListener(consumerGroup =
        "lifepark_group_data_align_" + MQConstants.TOPIC_COUNT_FOLLOWING, // Group 组
...
public class TodayUserFollowIncrementData2DBConsumer implements RocketMQListener<String> {
    ...
    public void onMessage(String body) {
        ...
        if (Objects.equals(result, 0L)) {
            ...
            //数据库写入成功后，再添加布隆过滤器中
            Long result2 = redisTemplate.execute(bloomAddScript, Collections.singletonList(userBloomKey), userId);
            log.info("源用户关注布隆过滤器添加结果：{}",result2);
        }
        ...
        // 布隆过滤器判断该日增量数据是否已经记录
        Long result3 = redisTemplate.execute(script, Collections.singletonList(targetUserBloomKey), targetUserId);
        log.info("目标用户粉丝布隆过滤器结果：{}" ,result3);
        // 若布隆过滤器判断不存在（绝对正确）
        if (Objects.equals(result, 0L)) {
            // 若无，才会落库，减轻数据库压力
