...
import com.lifepark.data.align.domain.mapper.DeleteMapper;
import com.lifepark.data.align.domain.mapper.SelectMapper;
import com.lifepark.data.align.domain.mapper.UpdateMapper;
...
public class NoteLikeCountShardingXxlJob {
    ...
    public void noteLikeCountShardingJobHandler() throws Exception {
        ...
        // 死循环
        for (;;) {
            // 1. 分批次查询 t_data_align_note_like_count_temp_日期_分片序号
            // ，如一批次查询 1000 条，直到全部查询完成
            List<Long> noteIds = selectMapper
                    .selectBatchFromDataAlignNoteLikeCountTempTable(tableNameSuffix, batchSize);
